...
export class DashboardComponent implements OnInit, OnDestroy{
  ...
  constructor(
    ...
    this.selectUserSubscription = this.store.pipe(select(selectTotalUsers)).subscribe((users) => {
      this.users$ = users as User[];
    });

    this.selectProfessionalSubscription = this.store.pipe(select(selectTotalProfessional)).subscribe((professionals) => {
      this.professionals$ = professionals as Professional[];
    });
    this.selectRequestProfessionalSubscription  = this.store.pipe(select(selectTotalRequestProfessional)).subscribe((professionals) => {
      this.requestProfessionals$ = professionals as Professional[];
    });
    this.selectLoadingSubscription = this.store.pipe(select(selectLoadingTotalUsers)).subscribe((loading) => {
