...
import {
  selectCompletedTaskData,
  selectCompletedTaskLoading,
  selectCompletedTotalTask,
...
export class TaskCompletedComponent implements OnInit, OnDestroy {
  ...
  constructor(private readonly _store: Store) {
    this.taskSubscription = this._store
      .pipe(select(selectCompletedTaskData))
      .subscribe((tasks) => {
        this.tasks = tasks;
      });
    this.loadingSubscription = this._store
      .pipe(select(selectCompletedTaskLoading))
      .subscribe((loading) => {
        this.loading$ = loading;
      });
    this.totalSubscription = this._store
      .pipe(select(selectCompletedTotalTask))
      .subscribe((total) => {
