...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.dataSubscription = this._professionalService
      .getProfessionalData()
      .subscribe(
        (data) => {
          this.userData = data;
          ...
          this.reviewSubscription = this._professionalService
            .getReviews(data._id, this.pageCount)
            .subscribe((reviews) => {
  ...
  submit(field: string) {
    if (
      ...
      this.validation = '';
      this.fileSubmitSubscription = this._professionalService
        .updateProfessional(this.userData)
        .subscribe((data) => {
  ...
  sendVerifyUser() {
    this.loading = true;
    this.sendVerifyUserSubscription = this._professionalService
      .sendVerifyUser()
      .subscribe(
        (data) => {
  ...
  onFileSelected(e: Event) {
    ...
    if (inputElement.files) {
      ...
      this.selectFileSubscription = this._professionalService
  ...
  nextPage() {
    ...
    this.nextPageSubscription = this._professionalService
      .getReviews(this.userData._id, page)
      .subscribe((reviews) => {
        this.reviews$ = reviews.data;
        this.pageCount++;
      });
  ...
  prevPage() {
    const page = this.pageCount - 1;
    this.prevPageSubscription = this._professionalService
      .getReviews(this.userData._id, page)
      .subscribe((reviews) => {
        this.reviews$ = reviews.data;
        this.pageCount--;
