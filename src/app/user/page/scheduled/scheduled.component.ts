...
export class ScheduledComponent implements OnInit, OnDestroy{
  ...
  constructor(
    ...
    this.meetingSubscription = this._store.pipe(select(selectInprogressScheduleData)).subscribe((meetings)=> {
    ...
    this.totalPageSubscription =this._store.pipe(select(selectInprogressTotalSchedule)).subscribe((total)=> {
