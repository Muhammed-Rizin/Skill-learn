...
export class ProfessionalsComponent implements OnInit, OnDestroy {
  ...
  constructor(private _store: Store) {
    this.professionalSubscription = this._store
      .pipe(select(selectProfessionalsData))
      .subscribe((professionals: professionalData[]) => {
    ...
    this.loadingSubscription = this._store
      .pipe(select(selectProfessionalsLoading))
  ...
  currentProfessionals() {
    ...
    this.professionals$ = this.totalProfessionals$.slice(
      this.skip,
      this.endIndex,
    );
  }
