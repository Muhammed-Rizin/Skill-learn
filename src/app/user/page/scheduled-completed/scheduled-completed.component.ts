...
export class ScheduledCompletedComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.meetingSubscription = this._store.pipe(select(selectCompletedScheduleData)).subscribe((meetings)=> {
      this.meetings = meetings
    })
    this.loadingSubscription = this._store.pipe(select(selectCompletedScheduleLoading)).subscribe((loading)=> {
      this.loading$ = loading
    })
    this.totalPageSubscription = this._store.pipe(select(selectCompletedTotalSchedule)).subscribe((total)=> {
