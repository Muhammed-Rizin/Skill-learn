...
export class ProfessionalProfileComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.route.params.subscribe((params) => {
      if (params['id']) {
        this.professionalDataSubscription = this.userService
          .getProfessionalDataById(params['id'])
          .subscribe(
            (data) => {
  ...
  private checkSubscriptionStatus(): void {
    ...
    this.userDataSubscription = this.userService
      ...
      .subscribe((data) => {
        ...
        this.subscribedSubscription = this.userService
          .subscribed(this.userId, this.userData._id)
          .subscribe(
            (data) => {
              if (data.createdAt) {
  ...
  reviewSubmit() {
    if (this.reviewForm.valid) {
      ...
      this.addReviewSubscription = this.userService
        .addReview(data, this.userData._id)
        .subscribe(
          (data) => {
  ...
  nextPage() {
    ...
    this.nextPageSubscription = this.userService
  ...
  prevPage() {
    const page = this.pageCount - 1;
    this.prevPageSubscription = this.userService
      .getReviews(this.userData._id, page)
      .subscribe((reviews) => {
        this.reviews$ = reviews.data;
        this.pageCount--;
